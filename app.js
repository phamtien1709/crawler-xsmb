  ...
  while (true) {
    await page.goto(
      `https://xoso.com.vn/xsmb-${
        crawingDate.getDate() >= 10 ? crawingDate.getDate() : '0' + crawingDate.getDate()
      }-${
    ...
    try {
      const allPrize = await page.evaluate(() => {
        const special = document.querySelectorAll('span.special-prize')[0].textContent.trim()

        const prize1 = document.querySelectorAll('span.prize1')[0].textContent.trim()
